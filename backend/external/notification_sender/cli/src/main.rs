...
use notification_sender::{actions, runner};
use std::str::FromStr;
use types::Error;
...
async fn main() -> Result<(), Error> {
    ...
    match command {
        "send" => {
            actions::send_notifications::run(&ic_agent, &index_store, &sms_sender, &email_sender)
                .await
        }
        "remove" => actions::remove_notifications::run(&ic_agent, &index_store).await,
